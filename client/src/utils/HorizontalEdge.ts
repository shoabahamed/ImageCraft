...
const fragmentSourceHorizontal = `
...
void main() {
    ...
    vec3 sum = vec3(0.0);
    vec2 offset;
    vec4 current_sample; // To fetch center pixel's alpha
    ...
    float edgeStrength = abs(sum.r); // Assuming grayscaled input, sum.r = sum.g = sum.b
    gl_FragColor = vec4(edgeStrength, edgeStrength, edgeStrength, texture2D(uTexture, vTexCoord).a);
}
`;

type HorizontalEdgeFilterOwnProps = Record<string, unknown>;

export class HorizontalEdgeFilter extends filters.BaseFilter<'HorizontalEdgeFilter', HorizontalEdgeFilterOwnProps> {
  static type = "HorizontalEdgeFilter";

  getFragmentSource() {
    return fragmentSourceHorizontal;
  }
  ...
  applyTo2d({ imageData: { data, width, height } }: T2DPipelineState) {
    const kernelY = [ // Sobel Gy for horizontal edges
      -1, -2, -1,
       0,  0,  0,
       1,  2,  1
    ];
      ...
      for (let c = 0; c < width; c++) {
        ...
        for (let ky = -halfKernel; ky <= halfKernel; ky++) {
          for (let kx = -halfKernel; kx <= halfKernel; kx++) {
            ...
            // Assuming grayscaled input (R channel)
        ...
        const magnitude = Math.min(255, Math.abs(gy));
        data[index] = magnitude;     // R
        data[index + 1] = magnitude; // G
        data[index + 2] = magnitude; // B
        // Alpha (data[index + 3]) is preserved from originalData
      }
    }
  }
}

classRegistry.setClass(HorizontalEdgeFilter);
